...
export default function BookingCalendar({ mode = 'booking', onNewBooking }) {
  ...
  const handleCheckIn = async () => {
    ...
    try {
      await api.post(`/api/bookings/${selectedBooking.id}/check-in`);
            ...
            <div className="p-4 border-t border-gray-200 space-y-2">
              {swapMode ? (
                ...
                <>
                  ...
                  {selectedBooking.status !== 'cancelled' && selectedBooking.status !== 'checked_out' && selectedBooking.status !== 'no_show' && (
